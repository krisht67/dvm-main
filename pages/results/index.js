    ...
    try {
      ...
      for (let i = 0; i < positionCount; i++) {
        const candidates = await this.state.ElectionInstance.methods
          ...
          .call()
        let sortedList = [...candidates]
          sortedList.sort((a,b)=>parseInt(b.voteCount)-parseInt(a.voteCount))
          candidatesTemp[i] = {
...
export function displayResults(candidates) {
  const renderResults = (candidate) => {
    return (
      <tr className="m-2">
        <td className="px-4">{candidate.candidateId}</td>
  ...
  return (
    <>
      ...
      <div className="container-main" style={{ borderTop: '1px solid' }}>
        <h2>Results</h2>
        <h2 className="">Total Posts: {candidates.length}</h2>
        {candidates.length < 1 ? (
          <div className="container-item attention">
          ...
          <>
            <div className="container-item flex flex-col">
                      {candidates.map((item,ind)=>{
                                ...
                                <p>{item?.pos}</p>
                                  <table>
                                    <tr>
                                      ...
                                      { item?.candidates?.map(renderResults)}
                                  </table>
                              </div>
                            )
                          }
