  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      const candidateCount = await this.state.ElectionInstance.methods
        ...
        this.setState({ candidatesList: candidatesTemp })
  ...
  render() {
    ...
    return (
      <>
        <Navbaradmin />
        <div className="">
          <h1 className="text-xl text-gray-900">Add a new candidate</h1>
        ...
        {loadAdded(this.state.candidatesList)}
      </>
    )
  }
}
        ...
        >
          {/* {candidates.map(renderAdded)} */}
          {
            candidates.map((item,ind)=>{
                ...
                {
                  return(
                    <div key={ind} className='flex flex-col text-white '>
                      <p>{item?.pos}</p>
                      { item?.candidates?.map(renderAdded)}
                    </div>
                  )
                }
